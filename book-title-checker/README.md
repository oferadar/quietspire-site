...
The source lives in the `book-title-checker/` folder of the [quietspire-site](https://github.com/oferadar/quietspire-site) repository, next to the static quietspire.com pages. That repository is served by GitHub Pages, which cannot run this app (the `/api/check` route and the data cache need a server), so the app deploys from Vercel: the project `book-title-checker` on the Quietspire Vercel account is connected to the repository with **Root Directory** `book-title-checker`, and every push to `main` that touches this folder deploys to production at https://book-title-checker.vercel.app. Add `NEXT_PUBLIC_SITE_URL` in the project's environment variables once a custom domain is attached (a subdomain such as titles.quietspire.com keeps it under the brand). GitHub Pages ignores the folder apart from serving its source files as plain text under quietspire.com/book-title-checker/, which is harmless because the repository is public anyway.

Open Library's front door intermittently refuses TCP connections, from home networks and from Vercel alike. `lib/booksource/openlibrary.ts` therefore makes up to three attempts of 7 s each, and the route handler exports `maxDuration = 30` so the attempts fit inside one invocation. Failures are logged server-side with their cause (`vercel logs`), while the client only ever sees the generic "temporarily unavailable" state.
